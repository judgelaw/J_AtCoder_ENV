...
| :--- | :--- | :--- |
| **C** | GCC-14 ||
| **C++** | G++-15 | 機械学習系などの一部ライブラリは省略|
| **Python** | 3.13.7 | pyenvによりバージョン固定 |
| **Rust** | 1.89.0 ||
...
1. Windowsのユーザーフォルダ（`C:\Users\<ユーザー名>`）に `.wslconfig` という名前のファイルを作成します。このリポジトリ内にも含めているので、必要があればコピーして使用するPCのCPU,メモリに合わせて内容を書き換えてください。書き換え後、ファイル保存時のエンコードはUTF-8としてください。
...
- **使用法**:ターミナルで`gen`コマンドを実行します。
 `gen [コンテスト名] [問題数と使用する言語(任意: c/cpp/py/rs)]`
  ...
  - Rustファイル作成時にはルート直下の`cargo.toml`ファイルへ`[[bin]]`を追記します。
  - ファイル作成数は引数に入力した数字に合わせて作成を行います。デフォルトでは`ABC`なら7つ、`AWC`なら5つ、`ADT`なら9つ、その他は6つ作成します。- ファイル作成はtemplateフォルダ内の各言語ファイルを複製します。
  - コンテスト名以外の引数はどの順番でも大丈夫です。
- **実行例**:
  - `gen abc350` : A-Gの問題ファイル (C++:gen.shで指定しているデフォルト言語) を作成。
  - `gen arc150 cpp py 3` : A-Cの問題ファイル (C++,Python) を作成。
